...
<script>
	...
	$: if (serialPort) //keep the same perchacnce
    {
	...
	function calcRefreshRate(arr){ //same
		let diff=arr[arr.length-1]-arr[0];
		return diff;
	}
	function calculateAverageChange(arr) { //same
		let difference = 0;
	...
	function convertTime(milliseconds) { //same
	...
	function updateDataFromSerialStream() { //same? we will see
		let newData = data;
		if (serialDataStream?.length > 0) {
			while (serialDataStream?.length > 0) {
				...
				try {
					...
					// console.log(decoded);
					Object.keys(logValues).forEach((logValue) => { //what is object
						if (!data[logValue]) {
							data[logValue] = [];
						}
